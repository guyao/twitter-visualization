...
def api():
    ...
    inx = label_vector[q]
    idx = calc_n_neighbor(inx[np.newaxis, :], X_sample)
    idx = idx[-N_neighbor:]
...
def neighbor(q=None):
    ...
    inx = label_vector[q]
    idx = calc_n_neighbor(inx[np.newaxis, :], X)
    idx = idx[-N_neighbor:]
...
def calc_n_neighbor(inX, X):
    # distances = sklearn.metrics.pairwise.euclidean_distances(X, inX)
    distances = sklearn.metrics.pairwise.cosine_distances(X, inX)
...
def plot_interactive_scatter(low_dim_embedding, labels, inx, q):
    ...
    # mark query
    c_x, c_y = inx[0]
    circle = Circle((c_x, c_y), 2, facecolor='none',
